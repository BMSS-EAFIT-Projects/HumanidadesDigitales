...
%	A partir de este contexto, la presente investigación busca responder la siguiente pregunta: ¿con qué temas, organizaciones, actores y eventos se asocia la ciencia en las columnas de opinión en Colombia, y qué patrones discursivos emergen de estas asociaciones? \textbf{PRECISAR PREGUNTA}
%	
%	Para ello, este artículo propone una estrategia basada en procesamiento de lenguaje natural que permite identificar menciones a la ciencia en una muestra de 13 mil columnas de opinión de tres periódicos colombianos de alta circulación y luego encontrar distintos elementos en el discurso, como las entidades o determinado vocabulario, con el fin de comprender cómo la ciencia es representada en el contexto colombiano.
	\textcolor{red}{A partir de este contexto, la presente investigación analiza una muestra de aproximadamente 13.000 columnas de opinión publicadas en tres medios de comunicación escritos colombianos de alta circulación, con el fin de responder a la siguiente pregunta: ¿con qué temas, organizaciones, actores y eventos se asocia la ciencia en el discurso de opinión en Colombia, y qué patrones discursivos emergen de dichas asociaciones? Para ello, el artículo propone una estrategia basada en técnicas de procesamiento de lenguaje natural que permite identificar las menciones explícitas a la ciencia en el corpus y examinar sus co-ocurrencias con entidades, actores y vocabulario específico, con el propósito de comprender cómo se construye y representa la ciencia en el espacio público colombiano.}
	...
	Todos estos análisis se llevaron a cabo con Python \textcolor{green}{poner la cita algo así como} \emph{Python} (Python Software Foundation, versión 3.x, 2016), 
...
%	Finalmente, se calcula la similitud de coseno entre el embedding de cada fragmento de las columnas y el embedding de cada subcategoría, con el fin de identificar la cercanía semántica.
	...
	\textcolor{red}{Una vez representados tanto los fragmentos del corpus como las subcategorías del TESAURO UNESCO en el mismo espacio vectorial mediante embeddings, fue necesario definir una métrica que permitiera cuantificar su cercanía semántica. La medida utilizada para evaluar la similaridad entre los textos codificados fue la similitud de coseno, una de las métricas más empleadas en recuperación de información y procesamiento de lenguaje natural (\cite{Jurafsky2025}).}
		
		\textcolor{red}{En este contexto, sea $\mathbf{u}$ el vector correspondiente a un fragmento (chunk) de una columna de opinión, y sea $\mathbf{v}$ el vector asociado a una de las 17 categorías del Tesauro UNESCO para Ciencias. La similitud de coseno entre ambos vectores se define como:}
...
\textcolor{red}{A partir de esta matriz, para cada fragmento $i$ se identificó la categoría con mayor valor de similitud, es decir,}

\[
j^*(i) = \arg\max_{j \in \{1,\dots,17\}} s_{i,j}.
\]

\textcolor{red}{Sin embargo, este criterio de asignación implica que todo fragmento queda clasificado en alguna categoría, incluso cuando sus niveles de similitud son bajos en términos absolutos. En otras palabras, el procedimiento de máximo relativo no distingue entre fragmentos que presentan una afinidad semántica significativa con el campo científico y aquellos cuya cercanía es débil o incidental.}

\textcolor{red}{Por esta razón, se introdujo un umbral de selección. Sea $\tau \in [0,1]$ dicho umbral. Un fragmento $i$ se considera una mención efectiva de ciencia únicamente si}

\[
\max_{j} s_{i,j} \geq \tau.
\]

\textcolor{red}{En caso contrario, el fragmento no se clasifica como mención científica, aun cuando tenga asignada una categoría por el criterio de máximo relativo. La definición adecuada de este umbral resulta crucial, pues permite maximizar la diferencia entre fragmentos que contienen referencias sustantivas al campo científico y aquellos que no lo hacen.}
	
	
%	Para determinar un umbral que permitiera maximizar la diferencia entre lo que se considera una mención de ciencia y lo que no, primero se calculó la similitud de coseno entre los embeddings de cada fragmento (Texto$_i$) y los de cada subcategoría de términos representativa de ciencia (Subcategoría$_j$). Este procedimiento generó la matriz de similitud $S$
%	
%	\begin{equation}
%		\mathbf{S} =
%		\begin{bmatrix}
...
\end{figure}
%	Para determinar este umbral, se clasificó manualmente una pequeña muestra de fragmentos mediante lectura cercana y se construyó una medida de accuracy por umbral (precisión, exactitud). Para la selección de los fragmentos, se eligieron aleatoriamente 10 por categoría. Previamente, se revisó la distribución de puntajes y se estimó que un valor adecuado para el filtro inicial sería mayor que 0.4. Con base en ello, se filtraron los fragmentos con puntaje superior a 0.4 y, a partir de ese conjunto, se seleccionaron aleatoriamente 10 fragmentos por subcategoría, obteniendo así un total de 170 fragmentos. 
	
%	Estos fragmentos fueron leídos manualmente y se les asignó una etiqueta de 1 si contenían una mención a ciencia y 0 en caso contrario; es decir, para cada fragmento $i$ se asignó un valor a la variable $y$,
%	
%	$$ y_i \in \{0,1\}.$$
%	
...
%	$$
%	
%	con incrementos de 0.01 (Ej: $(0, 0.01, 0.02,...,0.99, 1)$). Para cada valor de umbral $u$, se calculó la predicción
%	
%	$$
%	\hat{y}_i(u) = 
%	\begin{cases}
...
%	\]
%	
%	En este caso, el evento es \(\hat{y}_i(u) = y_i\). Luego el valor de $\mathrm{Acc}(u)$ más alto aproxima el mejor umbral $u$.
...
%	
%	La implementación de este modelo en Python permite controlar diversos pasos del proceso, como el tipo de modelo de embeddings a utilizar, así como modificar parámetros como el número máximo de tópicos que se desea obtener y el número mínimo de documentos necesarios para formar un tópico.
%	
%	Una estrategia para evaluar la calidad de los tópicos es la métrica de coherencia. Esta métrica se basa en principios probabilísticos y se ha estimado que valores más altos de coherencia se correlacionan con un mayor nivel de entendimiento de los tópicos generados según la opinión humana, como indican \cite{Rder2015}.
%	
%	El modelado se aplicó sobre todos los fragmentos utilizando el mismo modelo de embeddings empleado para la identificación de menciones a la ciencia: embeddinggemma-300m. Posteriormente, se aplicó una reducción de outliers con el objetivo de forzar al modelo a clasificar la mayor parte de los documentos.
%	
%	Para seleccionar el número óptimo de tópicos, el modelo se ejecutó con distintos valores de este parámetro y se calculó la métrica de coherencia para cada uno, lo que permitió aproximar el valor del parámetro correspondiente al mejor modelo.
%	
...
%	La Figura \ref{fig:dist_mensual_ciencia} muestra, para cada mes, el porcentaje de columnas con al menos una mención a la ciencia respecto al total, permitiendo observar su evolución a lo largo del período analizado. El mayor número de apariciones se concentra en marzo y abril de 2020, lo que sugiere que una proporción significativa de las menciones a la ciencia está asociada al inicio de la pandemia de COVID-19.

\textcolor{red}{La Figura~\ref{fig:dist_mensual_ciencia} presenta, para cada mes del período analizado, el porcentaje de columnas que contienen al menos una mención efectiva a la ciencia respecto del total de columnas publicadas en ese mes. Esta representación permite observar la evolución temporal de la presencia del discurso científico en la opinión pública. Se aprecia un incremento notable en marzo y abril de 2020, meses en los que se concentra el mayor porcentaje de menciones, lo que sugiere una asociación directa entre la intensificación del discurso científico y el inicio de la pandemia de COVID-19.
}
...
%		Se corrió el modelado de tópicos modificando el parámetro número de tópicos y se calculó el valor de coherencia para cada modelo. Se obtuvieron los resultados que se muestran en la tabla \ref{tab:coherence_topics}. 
%		
%		\begin{table}[H]
%			\centering
%			\begin{tabular}{ccccc}
...
%		\end{table}
%		
%		El número de tópicos finales es menor en una unidad en todos los casos respecto al número de tópicos definido por el parámetro, ya que al aplicar la reducción de outliers se elimina el tópico que agrupa a dichos documentos atípicos.
%		
%		Se observa que, al aumentar el número máximo de tópicos, el valor de la métrica de coherencia tiende a incrementarse. En particular, el número máximo de tópicos que el modelo generó de manera automática fue de 63.


\textcolor{red}{Inicialmente, este ejercicio de modelado de tópicos se realizó sobre el corpus completo, con el propósito de caracterizar la estructura temática general de las columnas de opinión antes de focalizar el análisis en el subcorpus de ciencia. Para ello, se ejecutó el modelo variando el parámetro \textit{num\_topics}, que define el número máximo de tópicos que pueden estimarse. Cada configuración del parámetro produce un modelo distinto, entendido aquí como una estructura específica de agrupación semántica en la que los fragmentos del corpus se distribuyen en un conjunto determinado de tópicos latentes.}

\textcolor{red}{Para cada modelo estimado se calculó la métrica de coherencia \textit{c\_v}, la cual evalúa el grado de consistencia semántica interna de los términos más representativos de cada tópico. En términos generales, valores más altos de coherencia indican que las palabras que definen un tópico tienden a coocurrir en el corpus, lo que suele asociarse con una mayor interpretabilidad desde el punto de vista humano. Reportar los resultados para distintas configuraciones permite comparar la calidad temática de los modelos y fundamentar la selección del número de tópicos más adecuado.}
...
\textcolor{red}{El número de tópicos finales es menor en una unidad respecto al valor inicialmente definido por el parámetro, ya que al aplicar la reducción de \emph{outliers} se elimina el tópico que agrupa los documentos considerados atípicos. Se observa que, en general, al aumentar el número máximo de tópicos, el valor de coherencia tiende a incrementarse, lo que sugiere una mayor homogeneidad semántica dentro de cada agrupación. En particular, cuando el modelo determina automáticamente el número de tópicos, se obtienen 63 tópicos finales y el mayor valor de coherencia entre las configuraciones evaluadas.}
	...
	\subsection{Análisis de vocabulario con POS-tagging}
%Se obtuvieron los verbos, adjetivos y sustantivos de los textos y se contaron sus frecuencias. Mediante POS-tagging se identificaron aproximadamente 10 mil verbos, 18 mil adjetivos y 30 mil sustantivos. El verbo más frecuente en todo el corpus es \textit{tener}, el adjetivo más frecuente es \textit{social} y el sustantivo más frecuente es \textit{país}. En el subconjunto de fragmentos con menciones a la ciencia, estas palabras siguen siendo las más frecuentes.
%
%A continuación, resulta importante observar de manera proporcional cuáles ocurren con mayor frecuencia en el subcorpus de ciencia. Como se mencionó en la sección \ref{sec:ident_menciones}, se identificó como menciones a la ciencia el 11.6\% de los fragmentos. Por lo tanto, es relevante analizar las palabras que ocurren en una proporción superior a ese valor y que, además, son frecuentes en el corpus general.
%
%En el caso de los verbos, aproximadamente 2500 ocurren con una proporción mayor al 11.6\%; en el caso de los adjetivos, son alrededor de 5 mil, y en el de los sustantivos, cerca de 8 mil. Muchas de estas palabras son poco frecuentes en el corpus, entonces se eligió un valor mínimo (por ejemplo al menos 50 apariciones en ciencia) por categoría gramatical. Tambien se filtró con una proporción del 30\% en frecuencia de ciencia sobre frecuencia total.

\textcolor{red}{Se extrajeron los verbos, adjetivos y sustantivos del corpus mediante técnicas de \emph{POS-tagging} y se calcularon sus frecuencias. En total, se identificaron aproximadamente 10 mil verbos, 18 mil adjetivos y 30 mil sustantivos distintos. En el corpus completo, el verbo más frecuente es \textit{tener}, el adjetivo más frecuente es \textit{social} y el sustantivo más frecuente es \textit{país}. Estas mismas palabras encabezan también las listas de frecuencia en el subcorpus de fragmentos con mención efectiva a la ciencia.}

\textcolor{green}{Dado que el 11.6\% de los fragmentos del corpus fueron clasificados como menciones científicas (véase sección \ref{sec:ident_menciones}), se seleccionaron aquellas palabras cuya proporción de ocurrencia en el subcorpus de ciencia supera ese porcentaje. Este criterio permite identificar términos con una presencia relativamente mayor en el discurso científico en comparación con el corpus general. A partir de este filtro se obtuvo el conjunto final de palabras analizadas en esta sección.}
...
%A partir del modelado de tópicos aplicado al subcorpus de fragmentos con menciones a la ciencia (Ver tabla~\ref{tab:topics_ciencia}), se identificaron como particularmente relevantes varios ejes temáticos recurrentes. Entre ellos destacan, en primer lugar, los temas asociados a la salud, con una presencia significativa de referencias a la pandemia de COVID-19, lo que refuerza la idea de que las coyunturas de crisis sanitaria aumentan la visibilidad de la ciencia en el discurso público. Junto a estos, emergen tópicos vinculados al desarrollo, la conservación ambiental y el cambio climático, así como menciones al medio ambiente desde una perspectiva más concreta, por ejemplo en relación con el agua, los ecosistemas y los recursos naturales.

\textcolor{red}{A partir del modelado de tópicos aplicado al subcorpus de fragmentos con menciones a la ciencia (véase Tabla~\ref{tab:topics_ciencia}), se identifican varios ejes temáticos claramente diferenciados. El tópico con mayor proporción (25.62\%) se articula en torno a categorías políticas e institucionales (política, gobierno, paz, seguridad), lo que sugiere que la ciencia aparece frecuentemente integrada en debates de carácter público y gubernamental más amplios. En segundo lugar (21.69\%), emerge un tópico asociado a salud y pandemia (salud, virus, crisis, ciencia), confirmando que la coyuntura del COVID-19 desempeñó un papel central en la visibilidad del discurso científico. También se identifican tópicos relevantes vinculados al desarrollo económico (11.42\%) y a la educación e investigación (9.61\%), lo que indica una conexión entre ciencia, innovación y formación superior. En el ámbito ambiental, aparecen de manera diferenciada tópicos relacionados con conservación y biodiversidad (6.56\%), cambio climático (4.79\%), recursos hídricos (2.75\%) y energía (2.02\%), lo que refuerza la idea de que las problemáticas ambientales constituyen un eje estructural en la articulación entre ciencia y opinión pública. En conjunto, estos resultados muestran que la ciencia no se presenta exclusivamente como contenido técnico especializado, sino como un recurso discursivo que atraviesa debates políticos, económicos, educativos y ambientales.}


%Asimismo, se identifican temas relacionados con la educación, la investigación y las universidades, lo que sugiere que la ciencia es frecuentemente asociada a las instituciones académicas como espacios legítimos de producción de conocimiento. Finalmente, aparecen tópicos vinculados a la energía, los combustibles y el petróleo, lo que evidencia la articulación de la ciencia con debates sobre modelos de desarrollo, transición energética y sostenibilidad, particularmente relevantes en el contexto colombiano.

...
\begin{document}
	...
	ii.	Tokens
	iii.	Stopwords
	iv.	Diferentes niveles de preprocesamiento (lematizado o no, chunks o no, sin stopwords o con stopwords)
	
	Cuando se analiza texto es necesario prepararlo y limpiarlo llevándolo al formato adecuado antes de aplicar los algoritmos de PLN [REF].
	Primero, en el texto de cada columna de opinión, se eliminaron urls, hashtags y arrobas (menciones a usuarios de redes sociales), también espacios en blanco extra y caracteres diferentes a letras, números y puntuación común. 
	
	\subsubsection{Fragmentación}
	
	
	\subsection{Identificación de menciones a la ciencia con búsqueda semántica}
	i.	Embeddings
	ii.	Similitud de coseno
	iii.	Tesauro Ciencia
	iv.	Elección umbral
	v.	Accuracy umbral
	
	\subsection{Análisis descriptivos y exploratorios de textos y autores (comparativos)}
	\subsection{Análisis de entidades – NER (comparativos)}
	\subsection{Análisis de temas (comparativos)}
	\subsection{Análisis léxico-discursivo/ vocabulario (keywords in context, concordancias, n-grams, POS-tagging) (comparativos)}
	...
	Primero para trabajar con las columnas de opinión es necesario aplicar preprocesamiento básico. Se llevó las fechas al formato requerido para Python. Se aplicó una limpieza que incluyó 
	...
	\begin{table}
		\begin{center}
			\input{tablas/tabla_frecuencias_columnas.tex}
		\end{center}
	\end{table}
